...
function urlDivName(string $idName = null): string {
    if (!strpos($_SERVER['HTTP_HOST'], "localhost")) {
        if ($idName) {
            return CONF_URL_BASE_TEST . "/#{$idName}";
        }
        return CONF_URL_BASE_TEST;
    }
    if ($idName) {
        return CONF_URL_BASE . "/#{$idName}";
    }
    return CONF_URL_BASE;
}
...
function theme(string $path = null): string {
    if (!strpos($_SERVER['HTTP_HOST'], "localhost")) {
        if ($path) {
            $url = CONF_URL_BASE_TEST . "/themes/" . CONF_VIEW_THEME . "/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
            return CONF_URL_BASE_TEST . "/themes/" . CONF_VIEW_THEME . "/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
        }
        return CONF_URL_BASE_TEST . "/themes" . CONF_VIEW_THEME;
    }
    if ($path) {
